...
def probe():  # pylint: disable=too-many-branches
  """ Find and import the lldb modules. This function tries to find the lldb module by:
      ...
      2. Executes the lldb executable pointed to by the LLDB environment variable (or if unset,
         the first lldb on PATH") with the -P flag to determine the PYTHONPATH to set. If the lldb
         executable returns a valid path, it's added to sys.path and the import is attempted again.
         If that fails, 3. On Mac OS X the default Xcode 4.5 installation path.
  """
